...
# === Load model ===
model = joblib.load('.venv/model.pkl')
...
# Image
st.image(".venv/ev-car-factory.jpg", use_container_width=True)
...
@st.cache_data
def load_data():
    df = pd.read_csv(".venv\preprocess_EV_data1.csv")
...
for i in range(1, forecast_horizon + 1):
    ...
    new_row = {
        ...
        'ev_pct_change_1': pct_change_1,
        'ev_pct_change_3': pct_change_3,
        'ev_growth_slope': ev_growth_slope
    }
    expected_features = model.feature_names_in_  # Available in sklearn 1.0+
    filtered_row = {k: v for k, v in new_row.items() if k in expected_features}
    pred = model.predict(pd.DataFrame([filtered_row]))[0]
